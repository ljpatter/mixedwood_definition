...
# Convert point data to sf and assign CRS (keeping your original sp/sf flow)
...
# Precompute row indices by block for TRUE block bootstrap
...
fit_gam_pdp <- function(df, n = 300) {
  ...
  df <- df %>% mutate(y_resp = exp(y))  # always inverse-link for Poisson
...
for (sp in species_list) {
  ...
  boot_results <- foreach(
    ...
    # Ensure reproducibility inside each worker
    set.seed(1000 + i)
    ...
    if (k_folds < 2L) return(NULL)  # guard: need at least 2 folds
    ...
    brt_model <- tryCatch({
      gbm.step(
        data             = sampled_data,
        gbm.y            = 1,
        gbm.x            = 3:ncol(sampled_data),   # INCLUDE year, coords, predictors
        family           = "poisson",
        tree.complexity  = 3,
        learning.rate    = 0.005,
        bag.fraction     = 0.5,
    ...
    # PDPs for ALL predictors (link scale from gbm::plot.gbm)
  ...
  if (length(pdp_all) > 0) {
    ...
    if (nrow(pdp_raw_sp) > 0) {
      pdp_raw_sp <- pdp_raw_sp %>%
        ...
        mutate(Species = sp)
      saveRDS(pdp_raw_sp, file.path(pdp_dir, paste0("pdp_bootstrap_raw_OG_NEW_", sp, ".rds")))
      ...
      saveRDS(pdp_gam_sp, file.path(pdp_dir, paste0("pdp_gam_summary_OG_NEW_", sp, ".rds")))
    }
  }
  ...
  if (k_folds_full < 2L) {
    message("Skipping final_model for ", sp, ": only ", length(uniq_blocks_full), " block(s).")
    final_model_list[[sp]] <- NULL
  } else {
    ...
    final_model <- gbm.step(
      data             = model_data,
      gbm.y            = 1,
      gbm.x            = 3:ncol(model_data),   # INCLUDE year, coords, predictors
      family           = "poisson",
      tree.complexity  = 3,
      learning.rate    = 0.005,
      bag.fraction     = 0.5,
...
# Save Results (all OG_NEW)
imp_df <- bind_rows(results_list)
friedman_df <- bind_rows(friedmanH_list)

saveRDS(results_list,     "Output/Tabular Data/results_list_final_parallel_poisson_OG_NEW.rds")
saveRDS(imp_df,           "Output/Tabular Data/imp_df_final_parallel_poisson_OG_NEW.rds")
saveRDS(friedman_df,      "Output/Tabular Data/friedman_interactions_bootstrapped_parallel_final_poisson_OG_NEW.rds")
saveRDS(final_model_list, "Output/Tabular Data/final_model_list_parallel_poisson_OG_NEW.rds")
saveRDS(imp_df, file.path(pdp_dir, "imp_table_OG_NEW.rds"))

cat("\n✅ Saved all model outputs for Poisson models with year as FACTOR (OG_NEW)!\n")
...
# ============================
# GAM-PDPs at Scale of Effect (OG_NEW)
# ============================
...
# --- Outputs ---
out_tab_dir <- file.path("Output/Tables",  paste0("GAM_PDP_SoE_", run_tag))
out_fig_dir <- file.path("Output/Figures", paste0("GAM_PDP_SoE_", run_tag))
...
# ---------- Load importance FROM OG_NEW ----------
imp_table_path_OG_NEW <- file.path(pdp_dir, "imp_table_OG_NEW.rds")
imp_df_path_OG_NEW    <- file.path(pdp_dir, paste0("imp_df_final_parallel_poisson_", run_tag, ".rds"))

if (file.exists(imp_table_path_OG_NEW)) {
  message("Loading importance from ", imp_table_path_OG_NEW)
  imp_table <- readRDS(imp_table_path_OG_NEW)
} else if (file.exists(imp_df_path_OG_NEW)) {
  message("imp_table_OG_NEW.rds not found; deriving from ", imp_df_path_OG_NEW)
  imp_table <- readRDS(imp_df_path_OG_NEW)
} else {
  stop("Could not find OG_NEW importance files:\n - ", imp_table_path_OG_NEW, "\n - ", imp_df_path_OG_NEW)
}
...
fit_gam_pdp_one <- function(df, n = n_points, trim = trim_prop) {
  ...
  df <- df %>%
    mutate(boot = factor(boot),
           y_resp = exp(y)) %>%      # always Poisson inverse link
  ...
  # Include dummy boot factor for predict()
  xnew <- tibble(
  ...
  tibble(
    ...
    edf = tryCatch({
      ...
      st[rownames(st) == "s(x)", "edf"] %||% NA_real_
    }, error = function(e) NA_real_),
    r2  = tryCatch(summary(g)$r.sq, error = function(e) NA_real_)
  )
}

# -------------------------------------------
# 2) Build GAM-PDPs only for those SoE features
#    using PDPs saved by the OG_NEW BRT run:
#    "pdp_bootstrap_raw_OG_NEW_<Species>.rds"
#    and SAVE ONE FILE PER PREDICTOR
# -------------------------------------------
all_plots <- list()

for (sp in species_list) {
  pdp_file <- file.path(pdp_dir, paste0("pdp_bootstrap_raw_OG_NEW_", sp, ".rds"))
  if (!file.exists(pdp_file)) {
    message("Missing PDP file for ", sp, ": ", pdp_file, " — skipping.")
    next
  }
...
p_all <- ggplot(all_tabs_df, aes(x = x, y = fit)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.25) +
  geom_line() +
  ggh4x::facet_grid2(rows = vars(Species), cols = vars(var_col),
                     scales = "free", independent = "all") +
  labs(
    title = "GAM-smoothed PDPs at Scale of Effect",
    x = "Predictor value",
    y = "Expected count"
  ) +
  theme_minimal(base_size = 13) +
...
print(p_all)
ggsave(file.path(out_fig_dir, "GAM_PDP_SoE__combined_grid_OG_NEW.png"),
       p_all, width = 12,
...
final_model_list <- readRDS("Output/Tabular Data/final_model_list_parallel_poisson_OG_NEW.rds")
...
imp_df <- readRDS("Output/Tabular Data/imp_df_final_parallel_poisson_OG_NEW.rds")
