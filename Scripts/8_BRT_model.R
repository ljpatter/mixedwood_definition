...
study_area <- st_read("C:/Users/leona/OneDrive/Desktop/Spatial Data/Chapter 1 Study Area")
...
# --- Outputs ---
out_tab_dir <- "Output/Tables/GAM_PDP_5m"
out_fig_dir <- "Output/Figures/GAM_PDP_5m"
...
for (sp in species_list) {
  ...
  for (v in unique_vars) {
    ...
    ggsave(png_name, p, width = 12, height = 4, dpi = 600)
...
print(p_all)
ggsave("Output/Figures/GAM_PDP_5m/GAM_PDP_SoE__combined_grid_freeY_5m.tiff",
       plot = p_all, device = "tiff", width = 20,
...
# ============================================================
# 3. Clean and classify predictors
# ============================================================
imp_df_clean <- imp_df %>%
...
# ============================================================
# 4. Helper to build plotmath labels
#    This will ACTUALLY bold the chosen labels
# ============================================================
make_label_expr <- function(features, is_bold) {
  out <- vector("list", length(features))
  
  for (i in seq_along(features)) {
    if (is_bold[i]) {
      out[[i]] <- bquote(bold(.(features[i])))
    } else {
      out[[i]] <- features[i]
    }
  }
...
# ============================================================
# 5. Plotting function
#    Currently bolds the top-ranked predictor within each var_type
#    If you have a true SoE table, replace the is_soe logic
# ============================================================
make_vi_plot <- function(df_sp, show_x_title = FALSE) {
  
  df_sp <- df_sp %>%
    group_by(var_type) %>%
    arrange(desc(MeanGain), .by_group = TRUE) %>%
    mutate(is_soe = row_number() == 1L) %>%
    ungroup() %>%
    mutate(Feature = fct_rev(fct_inorder(Feature)))
  
  label_map <- make_label_expr(
    features = as.character(df_sp$Feature),
    is_bold  = df_sp$is_soe
  )
  ...
  ggplot(df_sp, aes(x = Feature, y = MeanGain, fill = var_type)) +
    ...
    labs(
      x = NULL,
      y = if (show_x_title) "Mean relative influence" else NULL
    ) +
    theme_minimal(base_size = 13) +
    theme(
      ...
      axis.text.y      = element_text(color = "black", hjust = 1, size = 11),
...
# ============================================================
# 6. Build species plots
# ============================================================
species_levels <- levels(droplevels(imp_df_clean$Species))

plots_with_legend <- lapply(seq_along(species_levels), function(i) {
  sp <- species_levels[i]
  make_vi_plot(
    df_sp = filter(imp_df_clean, Species == sp),
    show_x_title = (i == length(species_levels))
  )
})

names(plots_with_legend) <- species_levels

legend_grob <- cowplot::get_legend(plots_with_legend[[1]])

plots <- lapply(plots_with_legend, function(p) {
...
panels_with_tags <- Map(
  ...
  plots,
  paste0(letters[seq_along(plots)], ")")
)

# ============================================================
# 8. Stack species panels
# ============================================================
right_stack <- cowplot::plot_grid(
  plotlist = panels_with_tags,
  ncol = 1,
...
# ============================================================
# 9. Draw legend in top-right
# ============================================================
final_plot <- cowplot::ggdraw(main_body) +
  cowplot::draw_grob(
    legend_grob,
    x = 0.76,
    y = 0.87,
...
# 10. Save
# ============================================================
out_fig <- "Output/Figures/VI_all_species_5m_combined.tiff"
...
##################### Poisson BRT Variable Importance — per-species plots
...
# colour map (keeps your original colours)
fill_map <- c(
  "prop_con" = "forestgreen",
  "clumpy"   = "cornflowerblue",
  "age_mn"   = "lightcoral"
...
# NEW group ordering requested by you: clumpy (top), prop_con (middle), age_mn (bottom)
var_order     <- c("clumpy", "prop_con", "age_mn")

# Set species order you want to appear (adjust if needed)
species_order <- c("BTNW","TEWA","BBWA")
...
imp_df_clean <- imp_df %>%
  ...
  mutate(
    ...
    # enforce the requested var_type order (controls facet order)
    var_type = factor(var_type, levels = var_order),
    # enforce species order (keeps only species you specified in that order)
    Species = factor(Species, levels = intersect(species_order, unique(Species)))
  ) %>%
  filter(!is.na(var_type), !is.na(Species))

# --- Fixed x axis across all panels (keep as you had; change if wanted) ---
x_max    <- 25
x_breaks <- seq(0, x_max, by = 5)

# --- Helper: convert names for display:
#     - _LULC -> _5m
#     - _NTEMS -> _30m
#     - add 'm' after any numeric scale token (e.g., 500 -> 500m)
make_display_name <- function(f) {
  f <- as.character(f)
  # explicit suffix conversions first
  f <- stringr::str_replace_all(f, "_LULC$", "_5m")
  f <- stringr::str_replace_all(f, "_NTEMS$", "_30m")
  # add 'm' to numeric tokens between underscores or at end
  f <- stringr::str_replace_all(f, "_(\\d+)_", "_\\1m_")
  f <- stringr::str_replace_all(f, "_(\\d+)$", "_\\1m")
  f
}

# --- One-species plot function (keeps legend) ---
#   Bars within each var_type are ordered descending by MeanGain (highest at top).
make_vi_plot <- function(df_sp, show_bottom_title = TRUE) {
  # arrange so within each var_type we go ascending MeanGain (so the last factor level is highest)
  df_sp <- df_sp %>%
    group_by(var_type) %>%
    arrange(MeanGain, .by_group = TRUE) %>%   # ascending: last = highest
    mutate(
      is_top        = row_number() == n(),    # highest within the group
      Feature_chr   = as.character(Feature),  # keep as character for mapping
      Feature_label = make_display_name(Feature_chr),
      Feature_label = ifelse(is_top, paste0("**", Feature_label, "**"), Feature_label),
      # set factor levels in the order produced by the arrange above so ggplot draws them
      Feature = factor(Feature_chr, levels = unique(Feature_chr))
    ) %>%
    ungroup()
  
  # mapping from Feature (factor value) -> display label (markdown-capable)
  ...
  ggplot(df_sp, aes(x = Feature, y = MeanGain, fill = var_type)) +
    ...
    coord_flip() +
    # facet rows ordered by var_type factor levels (we set var_order above)
    facet_grid(var_type ~ ., scales = "free_y", space = "free_y") +
    labs(
      x = NULL,
    ...
    theme(
      # remove grids for a cleaner look
      ...
      axis.text          = element_text(color = "black"),
      # allow bold markdown in the y-axis labels for the top item
      axis.text.y        = ggtext::element_markdown(hjust = 1, size = 12),
      axis.title.y       = element_text(margin = margin(r = 10)),
      
      # hide facet (group) strip text since we're already grouping visually
      strip.text.y       = element_blank(),
      strip.background   = element_blank(),
...
# --- Save each species as its own file (with legend included) ---
species_levels <- levels(imp_df_clean$Species)
out_dir <- "Output/Figures/VI_by_species_5m"
...
for (sp in species_levels) {
  ...
  p <- make_vi_plot(df_sp, show_bottom_title = TRUE) +
    labs(title = sp)
  
  out_fig <- file.path(out_dir, paste0("VI_", sp, "_5m.png"))
  ggsave(out_fig, p, device = "tiff", width = 18, height = 12, units = "cm", dpi = 600)
  message("✅ Saved: ", out_fig)
}

message("All per-species plots saved to: ", out_dir)
...
out_fig <- "Output/Figures/BBWA_propcon_clumpy_surface.tiff"
...
################ Moran's I for Poisson BRTs


# QPAD setup + offset helpers
