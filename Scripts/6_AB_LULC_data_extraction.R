...
#LULC <-rast("Input/Spatial Data/AB_LULC/AB_LULC_2020_10tm.tif")
...
# Save deciduous reclassification scheme in matrix
#rcl_matrix_dec <- matrix(
#  c(
#    1, 0,  # Water
#    2, 0,  # Bryoids
#    3, 0,  # Wetland – Treed
...
# Reclassify to deciduous
#LULC_reclassified_dec <- classify(LULC, rcl_matrix_dec)

# Save the reclassified rasters
#writeRaster(LULC_reclassified_con, "Output/Spatial Data/reproj_AB_LULC/Conifer_AB_LULC_2020_10tm_reclass.tif", overwrite = TRUE)
#writeRaster(LULC_reclassified_dec, "Output/Spatial Data/reproj_AB_LULC/Decid_AB_LULC_2020_10tm_reclass.tif", overwrite = TRUE)





############# Calculate proportion conifer and broadleaf
...
# Load rasters
Broadleaf_10TM <- rast("Output/Spatial Data/reproj_AB_LULC/Decid_AB_LULC_2020_10tm_reclass.tif")
Conifer_10TM <- rast("Output/Spatial Data/reproj_AB_LULC/Conifer_AB_LULC_2020_10tm_reclass.tif")
...
calculate_proportions <- function(buffer) {
  lapply(1:nrow(buffer), function(i) {
    ...
    return(c(conifer_prop = conifer_prop, broadleaf_prop = broadleaf_prop))
  })
}
...
age_stats_150 <- calculate_age_stats(buffer150)
age_stats_500 <- calculate_age_stats(buffer500)
age_stats_1000 <- calculate_age_stats(buffer1000)
